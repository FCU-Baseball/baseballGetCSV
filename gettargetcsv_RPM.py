...
def five():
    ...
    save_resultcsv_path = r'D:\Model_data\processed\csv'
...
def fivefold_175fps():
    ball_path = r'C:\Users\maxchen\modeldata\175fps_ball_line'
    #kfold_csv = pd.read_csv(r"D:\Model_data\processed\csv\5fold.csv") # 如果有5fold.csv跑這個
    kfold_csv = kfold(ball_path)  # 如果沒有5fold.csv跑這個
...
def iphone():
    ball_path = r'D:\Model_data\08camtemp\iphone3_ball_line'
    save_resultcsv_path = r'D:\Model_data\08camtemp\iphone.csv'
    folder_list = os.listdir(ball_path)
    df = pd.DataFrame(columns=['first', 'second', 'third', 'fourth', 'fifth'])
    save_first = []
    save_second = []
    save_third = []
    save_fourth = []
